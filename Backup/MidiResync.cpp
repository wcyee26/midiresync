...
string iMidifilename = "The White Stripes Fell In Love With A Girl Cover.mid";
        ...
        //for (int eventIndex = 0; eventIndex < 201; eventIndex++) {
            ...
            if (iMidifile[trackIndex][eventIndex].isNoteOn()) {
                ...
                if (isSignatureKey(iMidifile[trackIndex][eventIndex - 2])) {
                    float signatureEventTime = iMidifile[trackIndex][eventIndex - 2].seconds;
                    ...
                    float allowedSignatureAnimOnTime = adjustedBOnTime - (2 * maxSignatureAnimTimeLengths[trackIndex])
                                              - blendOutTime;
                    float signatureAnimOnTime = signatureEventTime - (2 * maxSignatureAnimTimeLengths[trackIndex])
                                                - blendOutTime;
                    if (signatureAnimOnTime > allowedSignatureAnimOnTime) {
                        signatureAnimOnTime = allowedSignatureAnimOnTime;
                    }
