...
//string iMidifilename = "Motorhead Ace Of Spades.mid";
string iMidifilename = "Slipknot Psychosocial Cover.mid";

void outputMIDIFile();
void outputLog();
void outputLogFile();
bool isPreviousSameBeat(float beatEventTime, float prevBeatEventTime);
bool isNextSameBeat(float beatEventTime, float nextBeatEventTime);
    ...
    for (int trackIndex = 0; trackIndex < trackCount; trackIndex++) {    
        ...
        //for (int eventIndex = 0; eventIndex < 201; eventIndex++) {
            ...
            if (iMidifile[trackIndex][eventIndex].isNoteOn()) {
                ...
                // If beaterCount is 1, reduce the maximum time before the end of beat
                ...
                aTime = eventTime;
                // If the drummer has less than 2 beaters 
                // OR the next OR next next beats are double-beats
...
bool isPreviousSameBeat(float beatEventTime, float prevBeatEventTime) {
    return beatEventTime >= (prevBeatEventTime - sameBeatMarginTime)
        && beatEventTime <= (prevBeatEventTime + sameBeatMarginTime);
}

bool isNextSameBeat(float beatEventTime, float nextBeatEventTime) {
    return beatEventTime >= (nextBeatEventTime - sameBeatMarginTime)
        && beatEventTime <= (nextBeatEventTime + sameBeatMarginTime);
}
